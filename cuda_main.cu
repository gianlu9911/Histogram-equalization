...
void equalizeImageWithCUDAGrayscale(const cv::Mat& inputImage, int tile_width, int tile_height) {
    ...
    int channels = inputImage.channels();  // For grayscale, this should be 1
    ...
    size_t imageSize = width * height * sizeof(unsigned char);  // Grayscale has only one channel
    ...
    std::ofstream csvFile("../execution_times_cuda_grayscale.csv", std::ios::app);
    if (!csvFile.is_open()) {
    ...
    cudaEventRecord(stage_start);
    // Compute the CDF for the grayscale image
